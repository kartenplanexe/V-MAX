...
/** The production coordinator: all instances share drafts, quotas and idempotency receipts. */
export class DurablePlanning {
  constructor(readonly options: { database: PlanningDatabase; plan: (job: Record<string, unknown>) => Promise<unknown>;
    ...
    provider: IntentProvider; dailyIntentCalls: number; dailyPlanCalls: number }) {}
  ...
  private action(owner: string, id: string, action: 'get' | 'edit' | 'confirm' | 'calculate', input?: unknown) {
    return this.options.database.withOwner(owner, async (state, save, client) => {
      const sessions = this.sessions(state, save, () => this.options.database.reserve(client, 'plan', this.options.dailyPlanCalls));
      // Ownership is checked before validation and quota reservation.
  ...
  async start(owner: string, input: unknown) {
    ...
    return this.options.database.withOwner(owner, async (state, save, client) => {
      ...
      if (state.attempts.length >= 3) throw new InitialIntentError('INTENT_RATE_LIMIT', 429);
      if (Object.keys(state.receipts).length >= 128) throw new InitialIntentError('INTENT_BUSY', 429);
      ...
      try {
        ...
        const { context, result } = await this.options.database.withSlot(client, 'intent', async () => {
          ...
          const result = await parseInitialIntent({ ...context, now: new Date().toISOString(), userText: body.user_text, inputId: body.event_id }, async request => {
            await this.options.database.reserve(client, 'intent', this.options.dailyIntentCalls);
